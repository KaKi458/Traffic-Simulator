...
public class Car {
    ...
    private Car neighbourFront, neighbourBackLeft, neighbourBackRight, neighbourFrontLeft, neighbourFrontRight;
    private double distance, distFrontLeft, distFrontRight, distBackLeft, distBackRight;
    private double speedDifference,speedDiffFrontLeft, speedDiffFrontRight, speedDiffBackLeft, speedDiffBackRight;
    ...
    public Car(Road road, int lane, FuzzyModel fuzzyModel) {
        ...
        if(first == null || first.getX() > 150) {
            ...
            if(first != null && first.getSpeed() < 50)
                this.speed = first.getSpeed() + 10;
            else
                this.speed = road.getMaxSpeed(x);
        }
        else {
    ...
    public void accelerate() {
        ...
        if(road.getObstacle(this) != null) {        
            ...
            if(obstacle.getLane() == currentLane &&  x < obstacle.getX() && (obstacle.getX() - x) / Simulation.pixelsPerMeter < 150 && (neighbourFront == null || neighbourFront.getX() > obstacle.getX())) {
        ...
        if(road.getTrafficLights() != null && trafficLight != null) {
            ...
            if((neighbourFront != null && neighbourFront.getX() > tLX) || (neighbourFront == null && tLX >  x)) {
        ...
        if(currentLane.getNumber() == -1 && x < 900)
            maxSpeed = 50;
    ...
    private boolean isLaneChangePossible() {
        ...
        conditions[2] = road.getObstacle(this) != null && road.getObstacle(this).getLane() == currentLane && x < road.getObstacle(this).getX() && road.getObstacle(this).getX() - x <= 50;
        conditions[3] = currentLane.getNumber() == -1;
        conditions[4] = zipperMerge;
        conditions[5] = destination != currentLane.getSide();
        conditions[6] = neighbourFrontRight != null && neighbourFrontRight.getLaneNumber() == -1 && x > 1000;
    ...
    private boolean isHigherSpeedPossible() {
        ...
        boolean condition3 = speed < road.getPossibleSpeed(currentLane.getNumber() + 1, x) 
    ...
    private void findNeighbours() {
        ...
        if(obstacle != null && obstacle.getLane() == currentLane && (neighbourFront == null || obstacle.getX() < neighbourFront.getX())) {
            distance = (obstacle.getX() - (x + LENGTH)) / Simulation.pixelsPerMeter;
            speedDifference = - speed;
        }
    }
    ...
    private void checkZipperWant() {
        ...
        if(speed < 5 && trafficLight != Color.RED && (road.getObstacle(this) != null && road.getObstacle(this).getLane() == currentLane)) {
        //&& ((obstacle != null && obstacle.getLane() == currentLane) || (neighbourBackRight != null && neighbourBackRight.getSpeed() > speed + 10) || (neighbourBackLeft != null && neighbourBackLeft.getSpeed() > speed + 10))){
            zipperWant = true;
        } 
    }
    ...
    private boolean tryTurnLeft() {
        ...
        boolean condition2 = !(obstacle != null && obstacle.getLaneNumber() == currentLane.getNumber() + 1 && x < obstacle.getX());
        boolean condition3 = !(destination == 1);

        if(condition1 && condition2 && condition3) {
            if(neighbourFrontLeft == null && neighbourBackLeft == null)
                decision = true;

            else if(neighbourFrontLeft == null) {

                if((speedDiffBackLeft <= 0 && distBackLeft > minSafeDist) || (speedDiffBackLeft > 0 && distBackLeft >= getSafeDistance(speedDiffBackLeft))) 
                    decision = true;
            }
            else if(neighbourBackLeft == null) {

                if((speedDiffFrontLeft >= 0 && distFrontLeft > minSafeDist) || (speedDiffFrontLeft < 0 && distFrontLeft >= getSafeDistance(Math.abs(speedDiffFrontLeft))))
                    decision = true;
            }
                ...
                if(((speedDiffBackLeft <= 0 && distBackLeft > minSafeDist) || (speedDiffBackLeft > 0 && distBackLeft >= getSafeDistance(speedDiffBackLeft)))
                 && ((speedDiffFrontLeft >= 0 && distFrontLeft > minSafeDist) || (speedDiffFrontLeft < 0 && distFrontLeft >= getSafeDistance(Math.abs(speedDiffFrontLeft)))))
                    decision = true;
            }
        }
    ...
    private boolean tryTurnRight() {
        ...
        if(condition1 && condition2 && condition3) {
            ...
            if(neighbourFrontRight == null && neighbourBackRight == null)
                decision = true;

            else if(neighbourFrontRight == null) {

                if((speedDiffBackRight <= 0 && distBackRight > minSafeDist) || (speedDiffBackRight > 0 && distBackRight >= getSafeDistance(speedDiffBackRight)))
                    decision = true;
            }
            else if(neighbourBackRight == null) {

                if((speedDiffFrontRight >= 0 && distFrontRight > minSafeDist) || (speedDiffFrontRight < 0 && distFrontRight >= getSafeDistance(Math.abs(speedDiffFrontRight))))
                    decision = true;
            }
                ...
                if(((speedDiffBackRight <= 0 && distBackRight > minSafeDist) || (speedDiffBackRight > 0 && distBackRight >= getSafeDistance(speedDiffBackRight)))
                 && ((speedDiffFrontRight >= 0 && distFrontRight > minSafeDist) || (speedDiffFrontRight < 0 && distFrontRight >= getSafeDistance(Math.abs(speedDiffFrontRight)))))
                    decision = true;
            }
        }
    ...
    private void setColor() {
        ...
        if(isCarInfo) {
            ...
            if(speed > 1.1 * maxSpeed)
                color = Color.MAGENTA;
            else if(speed >= 0.8 * maxSpeed)
                color = Color.GREEN;
            else if(speed < 0.8 * maxSpeed && speed >= 0.6 * maxSpeed)
                color = Color.YELLOW;
            else if(speed < 0.6 * maxSpeed && speed >= 0.3 * maxSpeed)
                color = Color.ORANGE;
            else if(speed < 0.3 * maxSpeed)
                color = Color.RED;
    ...
    public void setCarInfo(boolean w) { isCarInfo = w; }
    ...
    public void decideZipperMerge(List<Car> cars) {
        ...
        if(canZipper) {
            zipperMerge = false;
            for(Car car : cars) {
                if(car.getLaneNumber() == currentLane.getNumber() - 1 && car.getX() > x + 5* LENGTH && car.getX() < x + 10* LENGTH && car.getSpeed() < speed) {
