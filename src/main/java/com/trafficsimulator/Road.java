...
public class Road {
  ...
  public void render(Graphics g) {
    ...
    if (extraLane != null) extraLane.render(g2d);
  ...
  public int getMaxSpeed(double x) {
    ...
    if (secondMaxSpeed == 0) return maxSpeed;
    else {
      if (x < secondSpeedLimitX) return maxSpeed;
      else return secondMaxSpeed;
    }
  }
  ...
  public Car getNeighbourLeftBack(Car car) {
    ...
    Car neighbour = null;
    for (Car otherCar : cars) {
      if (otherCar.getLane().getNumber() == (car.getLane().getNumber() + 1)
          && otherCar.getX() < car.getX()) {
  ...
  public double getAverageSpeed() {
    ...
    for (Car car : cars) {
      if (car.getX() > 0 && car.getX() < 1800) averageSpeed += car.getSpeed();
    }
  ...
  public double getPossibleSpeed(int lane, double x) {
    ...
    if (noOfCarsOnLane == 0) possibleSpeed = maxSpeed;
  ...
  public RoadLane getLane(int lane) {
    ...
    if (lane >= 0 && lane < lanes.size()) return lanes.get(lane);
    else if (lane == -1) return extraLane;
    return null;
  }
  ...
  private void readSpeedLimitImages() throws IOException {
    ...
    speedLimitsImages.put(
        30, ImageIO.read(Objects.requireNonNull(getClass().getResource("/img/speed30.png"))));

    speedLimitsImages.put(
        40, ImageIO.read(Objects.requireNonNull(getClass().getResource("/img/speed40.png"))));

    speedLimitsImages.put(
        50, ImageIO.read(Objects.requireNonNull(getClass().getResource("/img/speed50.png"))));

    speedLimitsImages.put(
        60, ImageIO.read(Objects.requireNonNull(getClass().getResource("/img/speed60.png"))));

    speedLimitsImages.put(
        70, ImageIO.read(Objects.requireNonNull(getClass().getResource("/img/speed70.png"))));
  }

  public void changeSecondSpeedLimitX() {
    secondSpeedLimitX = secondSpeedLimitImgX;
  }

  public void dragSecondSpeedLimitImg(double x) {

    if (x >= 800 && x <= 1500) secondSpeedLimitImgX = (int) x;
    else if (x < 800) secondSpeedLimitImgX = 800;
    else secondSpeedLimitImgX = 1500;
  }
  ...
  public int getDestinationPercentage() {
    return destinationPercentage;
  }

  public boolean isSecondSpeedLimit() {
    return secondMaxSpeed != 0;
  }
  ...
  public int getSecondSpeedLimitX() {
    return secondSpeedLimitX;
  }
}
